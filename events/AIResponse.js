const { Collection, Events, Message, ApplicationCommandOptionType, PresenceUpdateStatus, AttachmentBuilder, EmbedBuilder, MessageFlags } = require('discord.js');
...
module.exports = {
    ...
    async execute(message, attempt = 0){
        attempt ++;
        try {
            if(message.flags.has(MessageFlags.SuppressNotifications) || !message.content || !(message.content?.includes(`<@${message.client.user.id}>`) || message.content?.includes(`<@!${message.client.user.id}>`)) || message.author.bot) return;
            ...
            let allowedMentions = {users: [message.author.id], roles: []};
            let msg;
